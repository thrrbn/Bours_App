"""
Orchestrateur - copie quasi-identique de `tools/backtest_analyst/analyst.py`
(16/08/2026, deja durci sur 3 bugs reels observes en usage avec llama3.1 -
voir son historique complet pour le detail de chaque correctif). Dupliquee
ici plutot qu'importee, meme raisonnement que quant_facts.py : `backend/app`
n'importe jamais `tools/` (voir docs/19-outils-pc-autonomes.md).

C'est le SEUL endroit ou le prompt envoye au LLM est construit, et le SEUL
endroit ou sa reponse est validee avant d'etre presentee - toute la
discipline decidee lors de la conversation du 14-16/08/2026 (faits
precalcules uniquement, citations obligatoires, seuil minimum de
transactions, regime best/worst determine par code, contenu chiffre exige)
est appliquee ici.

IMPORTANT pour la maintenance : toute correction future d'un probleme de
qualite du LLM (nouveau bug reel observe) doit etre appliquee ICI ET dans
`tools/backtest_analyst/analyst.py` - les deux copies ont volontairement
divergence-zero au moment de leur creation, mais rien ne les synchronise
automatiquement (voir "point de vigilance" de docs/20-instance-locale-pc-mac.md)."""
...
from app.domains.llm_analyst.llm_provider import LLMProvider
...
5. IMPORTANT : le regime le PLUS performant est **"{best_regime}"** (rendement moyen {best_regime_return:+.2f}%) et le MOINS performant est **"{worst_regime}"** (rendement moyen {worst_regime_return:+.2f}%) - ces deux noms sont DEJA DETERMINES par calcul, tu ne les choisis pas et tu ne dois JAMAIS en mentionner un autre a leur place. "best_regime_comment" porte EXCLUSIVEMENT sur "{best_regime}", "worst_regime_comment" porte EXCLUSIVEMENT sur "{worst_regime}".
...
def _best_worst_regime(facts: dict) -> tuple[dict, dict]:
    ...
    `facts["regime_ranking"]` (trie du meilleur au pire). Cas limite : un
    seul regime avec des transactions -> best == worst. Aucun regime avec de
    transactions -> regime a None, gere par les appelants."""
...
def _validate_citations(llm_data: dict, facts: dict) -> list[str]:
    ...
    seulement qu'il existe. `best_regime_comment`/`worst_regime_comment`
    verifient que chaque champ ne cite que des transactions appartenant
    reellement au regime qui lui est assigne (filet de securite si le
    modele ignore la consigne). `losing_trade_patterns` verifie l'is_win."""
...
def _validate_summary(summary: str) -> list[str]:
    """Detecte un resume "paresseux" (observe en usage reel avec llama3.1,
    une pure reformulation du titre, zero chiffre) - verifie mecaniquement
    qu'au moins un signe "%" est present, sans essayer de comprendre le
    texte (seule la PRESENCE est verifiee, pas l'exactitude)."""
    if "%" not in summary:
        return [
...
def _validate_regime_comment(item: dict, section: str) -> list[str]:
    """Meme logique que `_validate_summary()`, appliquee aux deux champs
    fixes du regime : en usage reel, une fois le bug de mauvais etiquetage
    corrige, le modele s'est mis a produire des "claim" corrects mais vides
    de contenu - verifie mecaniquement la PRESENCE d'au moins un chiffre."""
